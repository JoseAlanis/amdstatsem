...
theme_set(
  theme_linedraw() +
    theme(panel.grid = element_line(color = 'gray80'),
          panel.border = element_rect(size = rel(1), 
                                      color = 'gray'),
...
plt <- ggplot(data = data, aes(x = unlust, y = leistung)) +
  ...
  geom_curve(
    ...
    data = df, color = 'red', linewidth = 1.0,
...
df <- data.frame(x1 = -2.8, x2 = -0.2, y1 = 7, y2 = 21)
df_2 <- data.frame(x1 = 1, x2 = 5, y1 = 7, y2 = 14)
plt_znt <- ggplot(data = data_znt, aes(x = unlust, y = leistung)) +
  ...
  geom_curve(
    aes(x = x1, y = y1, xend = x2, yend = y2),
    data = df_2, color = 'blue', linewidth = 1.0,
    arrow = arrow(length = unit(0.03, "npc")),
    curvature = 0.25
  ) +
  ...
  annotate("rect", 
           ...
           ymin = 3, ymax = 7,
           alpha = .2, color = 'blue', fill = NA) +
  geom_point(data = data.frame(leistung = 21.1, unlust = 0),
             color = 'red', size = 2)
...
mlm_1 <- lmer(data = data_schulleistung,
              formula = leistung ~ unlust_c + (1 | klasse_nr) + (0 + unlust_c | klasse_nr))
summary(mlm_1)
ranef(mlm_1)

mod_1 <- lm(data = data_schulleistung, formula = leistung ~ unlust_c)
summary(mod_1)
