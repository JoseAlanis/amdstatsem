		...
		if (strcmp (tokens[0], "set") == 0) {
			...
			if (!params[i].name) {
				if (strcmp(tokens[1], "z")==0) {
					file_error(file,
							   "parameter z is deprecated. "
							   "Please use zr instead");
				} else {
					file_error(file,
							   "invalid parameter '%s'",
							   tokens[1]);
			...
			if (strcmp(tokens[1], "ks") == 0) {
				ex->method = METHOD_KS;
			} else if (strcmp(tokens[1], "ml") == 0) {
				ex->method = METHOD_ML;
			} else if (strcmp(tokens[1], "cs") == 0) {
				ex->method = METHOD_CS;
			} else {
				file_error(file,
						   "invalid method '%s' (must be 'ks', 'ml' or 'cs')",
						   tokens[1]);
			}
		} else {
 ...
 */
{
	...
	switch (ex->method) {
	case METHOD_KS:
		printf("  minimising the Kolmogorov-Smirnov distance\n");
		break;
	case METHOD_ML:
		printf("  maximising the likelihood\n");
		break;
	case METHOD_CS:
		printf("  minimising the Chi-Square value\n");
		break;
	}
...
experiment_log (struct experiment *ex, const struct dataset *ds,
				double *values, double *z, double penalty, double fit, double time)
 ...
 */
{
	...
	if (needs_header_line) {
		for (i=0; i<fields->used; ++i)
			fprintf (fd, " %8s", fields->entry[i]);
		fputc ('\n', fd);
	}

	for (i=0; i<fields->used; i++) {
		if (strcmp(fields->entry[i], "dataset") == 0) {
			fprintf (fd, " %8s", ds->key);
		} else if (strcmp(fields->entry[i], "method") == 0) {
			fprintf(fd, " %8s", method_name(ds->method));
		} else {
			int  found;
			found = dataset_get_value (ds, fields->entry[i],
									   values, z, penalty, fit, time, &value);
			if (found)
				fprintf (fd, " %8.4f", value);
			else
				fprintf (fd, " %8s", "-");
		}
	}
