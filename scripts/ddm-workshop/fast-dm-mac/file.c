...
new_file (const char *fname)
{
	...
	file->token_pos = xnew(int, file->tokens_alloc);
...
{
	if (file->line_used > 0) file_store_char(file, '\0');
	if (file->tokens_used >= file->tokens_alloc) {
		file->tokens_alloc += 80;
		file->token_pos = xrenew(int, file->token_pos,
								 file->tokens_alloc);
		file->tokens = xrenew(const char *, file->tokens,
							  file->tokens_alloc);
	}
	file->token_pos[file->tokens_used++] = file->line_used;
}
 ...
 * returns 0 on end of file.  Otherwise 1 is returned and '*w_ptr'
 * contains the '*n_ptr' tokens of a complete, non-empty input line.
 */
{
	...
	for (i=0; i<file->tokens_used; ++i) {
		file->tokens[i] = file->line + file->token_pos[i];
	}
	*w_ptr = file->tokens;
	*n_ptr = file->tokens_used;
	return (c != EOF || file->tokens_used > 0);
}
